...
AI 코딩 에이전트를 위한 런타임 프롬프트 게이트. 방금 입력한 모호한 명령을 —
루프 안에서, 에이전트가 잘못 이해하고 엉뚱한 파일을 고치기 전에 — 잡아낸다.
...
> **상태: 사전 릴리스(v0).** 소스에서 설치. `npx readback-gate` 패키지는 로드맵
> 참조([로드맵](#로드맵)).

**Node ≥ 24** 필요 (네이티브 TypeScript 실행 사용).

```sh
git clone https://github.com/dkwlsdl3/readback-gate
cd readback-gate
npm install -g .          # `readback-gate`, `readback-gate-codex` 설치
```
...
둘 다 된다. `readback-gate-codex`를 `UserPromptSubmit` 훅으로 등록한다 —
정확한 항목은 [install/README.md](install/README.md) 참조. 어댑터는:
...
- [ ] JS 빌드 스텝 + `npm publish` → `npx readback-gate`
- [ ] 전용 Claude Code 어댑터 (경계는 이미 분리돼 있음)
- [ ] 타당성 분석: 저점수가 실제 재작업과 상관 있나?
