...
## 13. 비실행 게이팅과 알려진 한계 (구현 후 추가, 2026-06-19)

구현·측정 과정에서 "비실행 프롬프트(질문/수락·잡담/순수 조회)"는 채점 대상이 아니라
**통과시켜야 한다**는 점이 확인됐다. 게이트의 목적은 *실행 직전 되묻기*이므로,
실행할 것이 없는 프롬프트에 개입하면 오발동(false positive)이다.

- **비실행 통과 규칙:** 질문어/`?`, 수락·잡담, 명시적 읽기전용 조회는 실행지향 감점
  (goal/target/done)을 면제하고 통과시킨다. 단 **변이 동사(mutating)가 있으면 통과 금지.**
- **위험 가드:** 조회처럼 보여도 변이/위험 동사가 있으면 우회하지 않는다. read-only 우회
  (`simpleReadOnly`)도 `!mutatingAction`으로 가드해, "상태 보여주고 삭제해줘"류가 새지 않게 한다.

측정(독립 라벨 코퍼스 + 픽스처):
- 비실행 과개입 FP **0%**, 현실혼합 정확도 **96.4%**, 실행 재현율 **100%** 유지.
- 고위험 파괴 명령을 조회어로 위장한 케이스(삭제/배포/drop/force push/초기화/rm -rf 등) **전부 차단**.

**알려진 한계 (v0, 안전 무관):**
- 변이 동사 판별은 denylist 기반이라 본질적으로 불완전하다. **위험으로 분류되지 않은**
  exotic 변이 동사(`rebase/squash/cherry-pick/checkout/flush/비활성화` 등)가 **조회어로 위장**되면
  nonExecution 경로로 통과(개입 누락)될 수 있다. 전부 복구 가능한 저위험 ops이며,
  파괴적(high-risk) 동사는 위험 분류로 차단된다.
- 근본 해결은 denylist 확장이 아니라 별도 설계(allowlist / 명령 절 분리)다 → v1 후보.

---

## 14. 타당성 백테스트 결과 (2026-06-19) — 전제 미입증

§9의 **타당성**("저점수가 실제 재작업과 상관 있나")을 실데이터로 처음 검증했다.

**방법:** 과거 세션 로그(Claude keeper-gen-2 + readback-gate, Codex)에서 사용자
프롬프트 ~3,666개를 readback-gate 스코어러로 재채점하고, 직후 구간에서 재작업
신호를 3개 감지기로 측정. (원문은 저장하지 않고 점수/신호만 집계.)

| 감지기 | 저점수 재작업률 | 고점수 재작업률 | 상대위험(저/고) |
|---|---|---|---|
| 교정 발언(다음 프롬프트 "아니/되돌려/wrong") | 7.2% | 9.3% | 0.77x |
| 강한 git 되돌림(revert / reset --hard / restore) | 0.3% | 0.3% | 1.12x(표본 미미) |
| 느슨(위 + 활동 혼동 포함) | 7.5% | 9.6% | 0.78x |

**결과:** 모든 감지기에서 상대위험 ≤ ~0.78. **저점수(모호) 프롬프트가 재작업을 더
부르지 않는다.** → clarity_score가 재작업을 예측한다는 전제는 **이 데이터로 미입증.**

**한계(과대해석 금지):**
- 후향적 outcome 감지는 혼동됨 — 교정/되돌림은 *구체적 시도*가 있어야 발생하므로
  명확한 프롬프트 쪽으로 편향된다. "무용지물"로 단정 불가, "근거 없음"이 정확.
- 이건 **상관**이지 **인과**가 아니다. 게이트가 꺼진 과거 로그라, *개입이 결과를
  개선하는가*(게이트의 실제 가치)는 측정 불가.

**결론 / 조치:**
- 포지셔닝을 정직화: "재작업 예측/오류 방지" 주장 제거 → **"실행 전 한 번 멈춰
  확인하게 하는 결정적 readback 도구"**로 한정(README 반영).
- 타당성은 **열린 질문**. 다음 진짜 검증 = **on/off A/B**(개입 유무로 재작업 비교).
- A/B가 긍정적이면, 그때 *무엇이 실제로 재작업을 예측하는지*에 맞춰 6축 재설계.
